...
export default function ModalCartComp({ item, handleClose, openModal, handleBtnKeranjang }) {
  ...
  function updateQty(type) {
    if (type == "+") {
      ...
      // jika - , cmn boleh diupdate max sampe 1 pengurangnya
      if (qty > 1) {
        setQty((prev) => prev-1);
      }
    }
    ...
    <>
      <Modal dismissible show={openModal} onClose={handleClose}>
          ...
          <div className="space-y-6">
           <div className="flex items-center space-x-4">
            ...
            <div className="flex justify-end mt-4">
              <ButtonGroup>
                <Button color="blue" onClick={() => updateQty("-")}>
                  <FaMinus classNamew-4 h-4 />
                </Button>
                <Button color="alternative">{qty}</Button>
                <Button color="blue" onClick={() => updateQty("+")}>
                  <FaPlus classNamew-4 h-4 />
                </Button>
              </ButtonGroup>
            </div>
          </div>
        ...
        <ModalFooter className="flex justify-end">
          ...
          <Button color="blue" onClick={() => handleBtnKeranjang(item, qty)}>
            Keranjang
          </Button>
        </ModalFooter>
      </Modal>
